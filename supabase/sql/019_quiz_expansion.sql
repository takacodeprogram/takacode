...
--
-- Note: dollar-quoting ($q$) est utilise pour encapsuler le JSONB
-- afin d'eviter les conflits avec les apostrophes du francais.
...
SET quiz = quiz || $q$[
  {"q": "Pourquoi un LLM peut-il donner des reponses differentes a la meme question ?", "choices": ["Parce que le modele est aleatoire : la temperature influence la variabilite", "Parce que le modele a ete reentraine entre-temps", "Parce que son entrainement change chaque jour"], "answer": 0, "explanation": "La temperature est un parametre qui controle le hasard dans la prediction du prochain token."},
...
SET quiz = quiz || $q$[
  ...
  {"q": "Pourquoi le francais utilise-t-il souvent plus de tokens que l anglais pour un meme texte ?", "choices": ["C est faux, c est l inverse", "Parce que les tokenizers sont optimises sur les corpus anglais majoritaires", "Parce que le francais est plus long"], "answer": 1, "explanation": "Les modeles sont entrainessurtout sur du texte anglais, donc les mots francais sont moins optimises."},
...
UPDATE track_lessons
SET quiz = quiz || $q$[
  {"q": "Peut-on changer le system prompt en cours de conversation ?", "choices": ["Non, il est definitif au demarrage", "Oui, on peut le modifier pour changer le comportement du modele", "Cela plante l IA"], "answer": 1, "explanation": "Tu peux modifier le system prompt a tout moment, cela re-cadre le comportement."},
...
SET quiz = quiz || $q$[
  ...
  {"q": "Combien d iterations faut-il viser en moyenne ?", "choices": ["Exactement 3, pas une de plus", "Autant que necessaire jusqu au critere de qualite defini au depart", "10 minimum pour etre professionnel"], "answer": 1, "explanation": "Le nombre ideal est celui qui atteint ton critere de qualite, ni plus ni moins."}
]$q$::jsonb
...
UPDATE track_lessons
SET quiz = quiz || $q$[
  {"q": "MCP est un protocole : que signifie cet acronyme ?", "choices": ["Model Context Protocol", "Machine Control Program", "Multi Code Platform"], "answer": 0, "explanation": "MCP = Model Context Protocol, un standard ouvert pour connecter l IA a des outils."},
...
SET quiz = quiz || $q$[
  ...
  {"q": "Ton outil IA propose du code qui semble correct mais tu as un doute. Que fais-tu ?", "choices": ["Tu l acceptes, l IA ne se trompe jamais", "Tu poses une question a l IA : explique ce code, pourquoi ce choix ? pour verifier", "Tu reinitialises tout le projet"], "answer": 1, "explanation": "Tu peux toujours demander a l IA d expliquer ce qu elle a fait. C est un excellent moyen d apprendre."},
...
UPDATE track_lessons
SET quiz = quiz || $q$[
  {"q": "Quel est le risque d accumuler du code IA sans jamais le relire ?", "choices": ["Le code peut etre vulnerable, fragile ou contenir des failles de securite sans que tu le saches", "Le code est toujours parfait", "L IA refuse d ecrire plus de code"], "answer": 0, "explanation": "Le code genere par IA doit etre audite regulierement, surtout avant mise en production."},
