...
//
// Providers supportes :
//   - gemini   (Google Gemini API - recommandé, free tier genereux)
//   - openrouter (OpenRouter - acces a de nombreux modeles)
//
// Configuration (env vars ou options explicites) :
//   AI_REVIEW_PROVIDER  = "gemini" | "openrouter"
//   AI_REVIEW_API_KEY   = cle API du provider
//   AI_REVIEW_MODEL     = modele (defaut: gemini-2.0-flash / google/gemini-2.0-flash-lite-free)
//
// Chaque provider a son propre free tier :
//   Gemini    → 60 req/min, 1M tokens/min (AI Studio) ; les donnees peuvent servir
//               a l'amelioration du modele. Ne pas y envoyer de code sensible.
//   OpenRouter → modeles "Free" limites a ~20 req/min, 200 req/jour.
//   Mistral   → tier experimentale gratuite avec Codestral (specialise code).
...
export function getAIReviewConfig(overrides = {}) {
  return {
    provider: overrides.provider || process.env.AI_REVIEW_PROVIDER || "gemini",
    model: overrides.model || process.env.AI_REVIEW_MODEL || "",
    apiKey: overrides.apiKey || process.env.AI_REVIEW_API_KEY || "",
    enabled: overrides.enabled !== undefined ? overrides.enabled : Boolean(process.env.AI_REVIEW_API_KEY)
  };
}

// Verifie si la config est valide pour effectuer une review.
export function isAIReviewAvailable(config) {
  return config.enabled && Boolean(config.apiKey) && Boolean(PROVIDER_CONFIG[config.provider]);
}
