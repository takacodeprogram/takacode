...
function normalizeLessonRow(row) {
  ...
  return {
    ...
    title: normalizeText(row.title, "Lecon"),
...
async function listLessonProgress(supabase, userId) {
  ...
  const { data, error } = await supabase
    ...
    .select("lesson_id, quiz_score, quiz_total, quiz_passed, project_submission, project_submitted_at, status, completed_at, xp_awarded")
    .eq("user_id", userId);
...
export async function getTrackCurriculum(supabase, trackId, userId) {
  ...
  for (const module of modules) {
    const moduleUnlocked = previousModulesCompleted;
    ...
    for (const lesson of module.lessons) {
      ...
      lesson.state = isCompleted ? "completed" : moduleUnlocked ? "available" : "locked";

      if (!nextLesson && moduleUnlocked && !isCompleted) {
    ...
    if (module.lessons.length > 0 && moduleCompletedCount < module.lessons.length) {
