...
function isMissingTrackSchemaError(error) {
  ...
  return (
    ...
    message.includes("user_track_enrollments") ||
...
export function mapTrackToRecommendation(track, fallbackTitle = "Parcours personnalise") {
  if (!track) {
    return {
      ...
      resources: ["Ressources en preparation"],
      objective: "Construire un projet concret.",
      nextSession: "Session annoncee bientot",
      nextSteps: [
  ...
  return {
    ...
    resources: resources.length ? resources : ["Ressources en preparation"],
    objective: normalizeText(track.objective, "Construire un projet concret."),
    nextSession: normalizeText(track.nextSession || track.next_session, "Session annoncee bientot"),
...
export function normalizeTrackRow(row) {
  ...
  return {
    ...
    summary: normalizeText(row.summary, "Parcours en preparation."),
    ...
    resources,
    nextSession: normalizeText(row.next_session, "Session annoncee bientot"),
    nextSteps,
...
export async function listPublishedTracks(supabase, options = {}) {
  ...
  if (error) {
    if (isMissingTrackSchemaError(error)) {
...
// Inscrit le membre a un parcours precis s'il ne l'est pas deja (idempotent).
// Utilise quand l'utilisateur ouvre une lecon ou demarre un parcours, pour que
// le parcours apparaisse immediatement dans son dashboard.
