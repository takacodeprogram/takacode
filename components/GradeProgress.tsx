    ...
    <article className="rounded-2xl border border-[var(--border-3)] bg-[var(--surface-1)] p-5">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-[10px] text-[var(--muted-4)] uppercase tracking-widest">Grade actuel</div>
            <div className="text-[14px] text-[var(--text-primary)] font-semibold leading-tight">{current.label}</div>
          </div>
        </div>
        <div className="text-right">
          <div className="text-[10px] text-[var(--muted-4)] uppercase tracking-widest">Points</div>
          <div className="text-[14px] text-[var(--text-primary)] font-semibold">{p}</div>
        </div>
      </div>
      ...
      {next ? (
        <>
          <div className="flex items-center justify-between text-[11px] mb-1.5">
            <span className="text-[var(--muted-3)]">Vers {next.label}</span>
            <span className="text-[#89c7ff] font-semibold">{pointsToNext} pts restants</span>
          </div>
          <div className="h-1.5 rounded bg-[var(--overlay-6)] overflow-hidden">
      ...
      {!compact ? (
        <p className="font-body-readable text-[11px] text-[var(--muted-2)] leading-snug mt-3">
          <span className="text-[var(--muted-3)]">Avantage {current.label} : </span>
          {current.perk}
        </p>
      ) : null}
    </article>
