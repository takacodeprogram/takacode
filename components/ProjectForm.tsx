...
export default function ProjectForm({ userId, tracks = [], project = null }: ProjectFormProps) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (isEdit) {
      ...
      setSaving(false);
      setMessage(t("projectForm.saved"));
  ...
  async function handleDelete() {
    if (!window.confirm(t("projectForm.deleteConfirm"))) {
      return;
    }
    setDeleting(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-white/[0.08] bg-[#111] p-5 space-y-4">
      ...
      <Field label={t("projectForm.fieldTitle")}><input className={INPUT} value={form.title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setField("title", e.target.value)} placeholder={t("projectForm.titlePlaceholder")} /></Field>
      <Field label={t("projectForm.fieldObjective")}><input className={INPUT} value={form.objective} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setField("objective", e.target.value)} placeholder={t("projectForm.objectivePlaceholder")} /></Field>
      <Field label={t("projectForm.fieldDescription")}>
        <textarea ref={descRef} className={`${INPUT} min-h-[90px]`} value={form.description} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setField("description", e.target.value)} placeholder={t("projectForm.descriptionPlaceholder")} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Field label={t("projectForm.fieldStatus")}>
        ...
        <Field label={t("projectForm.fieldDeadline")}><input type="date" className={INPUT} value={form.deadline || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setField("deadline", e.target.value)} /></Field>
      </div>
      ...
      <div className="rounded-xl border border-[#4F8EF7]/20 bg-[#4F8EF7]/[0.06] p-4 space-y-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-[#89c7ff] uppercase tracking-widest font-semibold">{t("projectForm.sectionAccelerator")}</span>
        </div>
        <p className="font-body-readable text-[11px] text-[#a5b8d8] leading-relaxed">{t("projectForm.acceleratorDesc")}</p>
        <select className={INPUT} value={form.track_id} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setField("track_id", e.target.value)}>
          <option value="">{t("projectForm.noTrack")}</option>
      ...
      <Field label={t("projectForm.fieldRevenueModel")}>
        <select className={INPUT} value={form.revenue_model} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setField("revenue_model", e.target.value)}>
          <option value="">{t("projectForm.revenueNone")}</option>
          <option value="vente">{t("projectForm.revenueVente")}</option>
          <option value="abonnement">{t("projectForm.revenueAbonnement")}</option>
          <option value="publicite">{t("projectForm.revenuePublicite")}</option>
          <option value="affiliation">{t("projectForm.revenueAffiliation")}</option>
          <option value="freelance">{t("projectForm.revenueFreelance")}</option>
        </select>
      </Field>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <Field label={t("projectForm.fieldLiveUrl")}><input className={INPUT} value={form.live_url} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setField("live_url", e.target.value)} placeholder={t("projectForm.liveUrlPlaceholder")} /></Field>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <button type="submit" disabled={saving} className={`btn-primary inline-flex items-center gap-2 text-[12px] ${saving ? "opacity-50 cursor-not-allowed" : ""}`} style={{ padding: "10px 18px" }}>
          {saving ? t("projectForm.saving") : isEdit ? t("projectForm.save") : t("projectForm.create")}
        ...
        {isEdit ? (
          <button type="button" onClick={handleDelete} disabled={deleting} className="text-[12px] text-red-400/80 hover:text-red-400 inline-flex items-center gap-1.5">
            ...
            {deleting ? t("projectForm.deleting") : t("projectForm.delete")}
          </button>
        ) : null}
      </div>
    </form>
