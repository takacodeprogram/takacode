...
export default function AffiliateBlock({ links = [], heading = "Fournisseurs recommandes", grouped = false }: Props) {
  if (!Array.isArray(links) || links.length === 0) {
    return null;
  }

  const groups: Record<string, AffiliateLink[]> = grouped
    ? links.reduce<Record<string, AffiliateLink[]>>((acc, link) => {
