...
// Propositions de parcours (mentors) a valider ou rejeter par l'admin.
export default function PendingTracksReview({ initialPending = [] }) {
  ...
  async function decide(trackId, approve) {
    setBusyId(trackId);
    setError("");
    ...
    if (rpcError || data?.error) {
      setError(rpcError?.message || data?.error || "Action impossible.");
      return;
    }
