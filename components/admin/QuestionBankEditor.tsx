...
export default function QuestionBankEditor({ lessonId, userId, objectives = [], resources = [] }: Props) {
  ...
  async function saveQuestion(question: QuestionRow) {
    ...
    toast(t("adminQuestions.saved"), "success");
  }

  async function deleteQuestion(questionId: string) {
    if (!window.confirm(t("adminQuestions.confirmDelete"))) return;
          ...
          {questions.map((question, questionIndex) => (
            <article key={question.id} className="rounded-xl border border-white/[0.07] bg-[#0d0d0d] p-4 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button type="button" onClick={() => saveQuestion(question)} disabled={busyId === question.id} className="btn-secondary text-[11px]" style={{ padding: "8px 12px" }}>{t("adminQuestions.save")}</button>
                <button type="button" onClick={() => deleteQuestion(question.id)} disabled={busyId === question.id} className="px-2 py-1 text-[11px] text-red-400/80 hover:text-red-300">{t("adminQuestions.delete")}</button>
              </div>
            </article>
          ))}
        </div>
