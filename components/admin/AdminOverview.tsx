...
function resolveUserLabel(user: Record<string, unknown>, t?: (key: string) => string): string {
  ...
  return id ? `${id.slice(0, 6)}...` : (t ? t("admin.member") : "Membre");
}
...
export default function AdminOverview({ users = [], tracks = [], platformStats = null, systemIssues = [], northStar = null }: Props) {
  ...
  const metrics: MetricTile[] = [
    { label: t("adminOverview.users"), value: listUsers.length },
    { label: t("adminOverview.admins"), value: listUsers.filter((entry) => isRoleAdmin(entry?.role)).length },
    {
      label: t("adminOverview.activeTracks"),
      value: listTracks.filter((entry) => entry?.is_active !== false && entry?.isActive !== false).length
    },
    {
      label: t("adminOverview.published"),
  ...
  const curriculumTiles: MetricTile[] = platformStats?.ready
    ? [
        { label: t("adminOverview.modules"), value: platformStats.totalModules || 0 },
        { label: t("adminOverview.lessons"), value: platformStats.totalLessons || 0 },
        { label: t("adminOverview.lessonsCompleted"), value: platformStats.completedLessons || 0 },
        { label: t("adminOverview.submittedProjects"), value: platformStats.submittedProjects || 0 }
      ]
    : [];
  ...
  const leaderboard: LeaderboardEntry[] = [...listUsers]
    .map((entry) => ({
      id: entry?.id,
      label: resolveUserLabel(entry as Record<string, unknown>, t),
      grade: typeof entry?.grade === "string" && entry.grade.trim() ? entry.grade : t("adminOverview.starter"),
    ...
    <div className="space-y-5">
      ...
      {systemIssues.length ? (
        <div className="rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3">
          <div className="text-[12px] text-amber-100 font-semibold mb-1">{t("adminOverview.actionRequired")}</div>
