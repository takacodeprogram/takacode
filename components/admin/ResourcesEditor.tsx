      ...
      {resources.map((res, index) => (
        <div key={index} className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <input className={INPUT} value={res.label} onChange={(e) => update(index, { label: e.target.value })} placeholder={t("adminResources.placeholderTitle")} />
            <input className={INPUT} value={res.url} onChange={(e) => update(index, { url: e.target.value })} placeholder={t("adminResources.placeholderUrl")} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input className={INPUT} value={res.why} onChange={(e) => update(index, { why: e.target.value })} placeholder={t("adminResources.placeholderWhy")} />
          <input className={INPUT} value={res.how} onChange={(e) => update(index, { how: e.target.value })} placeholder={t("adminResources.placeholderHow")} />
        </div>
      ))}
