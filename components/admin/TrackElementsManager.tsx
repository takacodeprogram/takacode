...
interface TrackElementsManagerProps {
  trackId: string;
  trackSlug: string;
...
export default function TrackElementsManager({ trackId, trackSlug, initialModules = [], basePath = `/admin/parcours/${trackId}` }: TrackElementsManagerProps) {
  ...
  const [createForm, setCreateForm] = useState<{ slug: string; title: string; summary: string; sort_order: string }>(EMPTY_MODULE);
  const [editingId, setEditingId] = useState<string>("");
  const [editForm, setEditForm] = useState<{ slug: string; title: string; summary: string; sort_order: string }>(EMPTY_MODULE);
  ...
  async function reload() {
    ...
    setModules(((mods || []) as { id: string; slug: string; title: string; summary?: string; sort_order: number; is_published: boolean; created_at: string }[]).map((m) => ({ ...m, lessons: byModule[m.id] || [] })));
    router.refresh();
  }
  ...
  return (
    <section className="space-y-4">
      ...
      {modules.map((module, index: number) => (
        <div key={module.id} className="rounded-2xl border border-white/[0.08] bg-[#111] p-4">
          {editingId === module.id ? (
            <div className="space-y-2.5 mb-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
                <input className={INPUT} value={editForm.title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditForm((c) => ({ ...c, title: e.target.value }))} placeholder="Titre" />
                <input className={INPUT} value={editForm.slug} disabled placeholder="slug" />
                <input className={INPUT} type="number" min="1" value={editForm.sort_order} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditForm((c) => ({ ...c, sort_order: e.target.value }))} placeholder="ordre" />
              </div>
              <input className={INPUT} value={editForm.summary} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditForm((c) => ({ ...c, summary: e.target.value }))} placeholder="Resume" />
          ...
          <div className="space-y-2 pl-1">
            {module.lessons.map((lesson: LessonEntry) => (
      ...
      <form onSubmit={handleCreateModule} className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4 space-y-2.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
          <input className={INPUT} value={createForm.title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCreateForm((c) => ({ ...c, title: e.target.value }))} placeholder="Titre du module" />
          <input className={INPUT} value={createForm.slug} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCreateForm((c) => ({ ...c, slug: e.target.value }))} placeholder="slug (ex: bases-html)" />
          <input className={INPUT} type="number" min="1" value={createForm.sort_order} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCreateForm((c) => ({ ...c, sort_order: e.target.value }))} placeholder="ordre" />
        </div>
        <input className={INPUT} value={createForm.summary} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCreateForm((c) => ({ ...c, summary: e.target.value }))} placeholder="Resume (optionnel)" />
