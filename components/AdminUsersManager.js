...
export default function AdminUsersManager({ initialUsers = [], currentUserId = "" }) {
  const supabase = useMemo(() => createClient(), []);
  const [users, setUsers] = useState(Array.isArray(initialUsers) ? initialUsers : []);
  const [pointsDraft, setPointsDraft] = useState(() =>
    Object.fromEntries((initialUsers || []).map((u) => [u.id, String(Number(u.points ?? 0))]))
  );
  const [roleDraft, setRoleDraft] = useState(() =>
    Object.fromEntries((initialUsers || []).map((u) => [u.id, typeof u.role === "string" ? u.role : "user"]))
  );
  const [savingPointsId, setSavingPointsId] = useState("");
  const [applyingRoleId, setApplyingRoleId] = useState("");
  const [confirm, setConfirm] = useState(null);
  const [errorMessage, setErrorMessage] = useState("");
  const [successMessage, setSuccessMessage] = useState("");

  async function handleSavePoints(userId) {
    ...
    setUsers((current) => current.map((u) => (u.id === userId ? { ...u, ...data } : u)));
    setPointsDraft((current) => ({ ...current, [userId]: String(data.points) }));
  ...
  function requestRoleChange(user) {
    ...
    setConfirm({ user, oldRole: user.role, newRole });
  }
  ...
  async function confirmRoleChange() {
    ...
    if (error || data?.error) {
      const code = data?.error || (error?.message?.includes("function") ? "rpc_missing" : "");
      setErrorMessage(ROLE_ERROR[code] || error?.message || "Impossible de changer le role.");
      setApplyingRoleId("");
      setConfirm(null);
      // reset le select a la valeur reelle
      setRoleDraft((current) => ({ ...current, [user.id]: user.role }));
      return;
    }
  ...
  return (
    <section className="rounded-2xl border border-white/[0.08] bg-[#111] p-5">
                    ...
                    {isSelf ? (
                      ...
                      <div className="flex items-center gap-1.5">
                        <select
                          ...
                          value={selectedRole}
                          onChange={(event) => setRoleDraft((c) => ({ ...c, [user.id]: event.target.value }))}
                        >
                          {ROLE_OPTIONS.map((r) => (
                  ...
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        onChange={(event) => setPointsDraft((c) => ({ ...c, [user.id]: event.target.value }))}
                      />
                      <button
                        type="button"
      ...
      {confirm ? (
        <div className="fixed inset-0 z-[55] flex items-center justify-center p-5" style={{ background: "rgba(0,0,0,0.62)", backdropFilter: "blur(4px)" }} onClick={() => setConfirm(null)}>
          <div className="celebrate-pop w-full max-w-[400px] rounded-2xl border border-white/[0.1] bg-[#111] p-6" onClick={(e) => e.stopPropagation()}>
