    ...
    <>
      ...
      {!schemaReady ? (
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[#111] p-10 flex flex-col items-center text-center gap-3">
          ...
          <div className="font-venite text-[15px] text-white">{t("dashboardSessions.emptyTitle")}</div>
          <p className="font-body-readable text-[13px] text-[#777] max-w-[460px]">{t("dashboardSessions.emptyDesc")}</p>
        </div>
      ) : (
        <div className="space-y-8">
          {upcoming.length ? (
            <section>
              <h2 className="font-venite text-[13px] tracking-widest text-[#888] mb-3">{t("dashboardSessions.upcoming")}</h2>
          ...
          {replays.length ? (
            <section>
              <h2 className="font-venite text-[13px] tracking-widest text-[#888] mb-3">{t("dashboardSessions.replays")}</h2>
              <div className="space-y-3">
                {replays.map((session) => (
                  <article key={session.id} className="rounded-2xl border border-white/[0.08] bg-[#111] p-5 flex items-center justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-[#666] font-body-readable mt-0.5">{formatWhen(session.scheduledAt, t)}</div>
                    </div>
