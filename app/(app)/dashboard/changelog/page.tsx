...
const STATUS_CLASSES: Record<string, string> = {
  ...
  planifiee: "border-[var(--border-4)] bg-[var(--overlay-3)] text-[var(--muted-3)]"
};
    ...
    <>
      ...
      <section className="rounded-2xl border border-blue-500/20 bg-gradient-to-br from-blue-500/10 via-[#111] to-violet-500/10 p-5 md:p-6 mb-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="font-venite-italic text-[18px] md:text-[22px] text-[var(--text-primary)] leading-tight mb-2">
              {t("dashboardChangelog.heading")}
            </h2>
            <p className="font-body-readable text-[12px] md:text-[13px] leading-relaxed text-[var(--muted-2)]">
              {t("dashboardChangelog.description")}
            </p>
          </div>
          <div className="shrink-0 rounded-2xl border border-[var(--border-3)] bg-[var(--surface-3)]/80 px-5 py-4 text-center">
            <div className="text-[10px] uppercase tracking-[0.18em] text-[var(--muted-4)] mb-1">{t("dashboardChangelog.currentVersion")}</div>
            <div className="font-valorax text-3xl text-[var(--text-primary)]">V{PRODUCT_RELEASES[0].version}</div>
          </div>
        </div>
      </section>
      ...
      <div className="space-y-4">
        {PRODUCT_RELEASES.map((release, index) => (
          <article key={release.version} className="project-card rounded-2xl border border-[var(--border-3)] bg-[var(--surface-1)] p-5 md:p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div className="flex min-w-0 items-start gap-4">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <h2 className="font-venite-italic text-[15px] text-[var(--text-primary)]">{release.title}</h2>
                    {index === 0 ? (
                  ...
                  <p className="font-body-readable text-[12px] leading-relaxed text-[var(--muted-2)]">{release.summary}</p>
                </div>
              </div>
              ...
              <div className="flex shrink-0 items-center gap-2 md:flex-col md:items-end">
                ...
                <time className="text-[10px] text-[var(--muted-4)]">{release.date}</time>
              </div>
            </div>

            <div className="mt-5 grid grid-cols-1 gap-2.5 md:grid-cols-2">
              {release.highlights.map((highlight) => (
                <div key={highlight} className="flex items-start gap-2.5 rounded-xl border border-[var(--border-2)] bg-[var(--overlay-2)] px-3.5 py-3">
                  ...
                  <span className="font-body-readable text-[11px] leading-relaxed text-[var(--muted-1)]">{highlight}</span>
                </div>
              ))}
            </div>
          </article>
