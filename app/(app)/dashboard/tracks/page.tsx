...
export async function generateMetadata() {
  ...
  return buildPageMetadata({
    title: t("dashboardTracks.title"),
    description: t("dashboardTracks.emptyDesc"),
...
export default async function MyTracksPage(props: { searchParams: Promise<{ page?: string }> }) {
  ...
  if (!user) {
    redirect("/signin?next=/dashboard/tracks");
  }
  ...
  for (const enrollment of enrollments) {
    ...
    cards.push({
      ...
      nextHref: curriculum.nextLesson
        ? `/tracks/${enrollment.track.slug}/lesson/${curriculum.nextLesson.lessonSlug}`
        : `/tracks/${enrollment.track.slug}`,
  ...
  return (
    <>
      <PageHeader
        title={t("dashboardTracks.title")}
        subtitle={`${cards.length} ${cards.length > 1 ? t("dashboard.tracks").toLowerCase() : t("dashboard.tracks").toLowerCase()} ${t("dashboardTracks.progress").toLowerCase()}`}
        actions={
            <Link href={localePath("/tracks", locale)} className="btn-secondary inline-flex items-center gap-2 text-[12px]">
            <iconify-icon icon="lucide:compass" style={{ fontSize: "13px" }} />
            {t("dashboardTracks.exploreCatalog")}
          </Link>
        }
      />
      ...
      {cards.length ? (
              ...
              <article key={track.id} className="rounded-2xl border border-white/[0.08] bg-[#111] p-5 card-hover project-card">
                ...
                <div className="flex flex-wrap gap-2.5">
                  <Link href={nextHref} className="btn-primary inline-flex items-center gap-2 text-[12px]" style={{ padding: "10px 16px" }}>
                    {started ? t("dashboardTracks.continue") : t("dashboardTracks.start")}
                    <iconify-icon icon="lucide:arrow-right" style={{ fontSize: "13px" }} />
                  </Link>
                  <Link href={localePath(`/tracks/${track.slug}`, locale)} className="btn-secondary inline-flex items-center gap-2 text-[12px]" style={{ padding: "10px 16px" }}>
                    {t("dashboardTracks.program")}
                  </Link>
                </div>
              </article>
            ))}
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[#111] p-10 flex flex-col items-center text-center gap-4">
          ...
          <Link href={localePath("/tracks", locale)} className="btn-primary inline-flex items-center gap-2" style={{ fontSize: "13px", padding: "12px 24px" }}>
            <iconify-icon icon="lucide:compass" style={{ fontSize: "14px" }} />
            {t("dashboardTracks.exploreTracks")}
          </Link>
        </div>
      )}
    </>
