...
export async function generateMetadata({ params }: LessonPageProps) {
  ...
  return buildPageMetadata({
    title: t("lessonPage.metadata.title"),
    description: t("lessonPage.metadata.description"),
    path: slug && lessonSlug ? `/tracks/${slug}/lesson/${lessonSlug}` : "/tracks",
    noIndex: true
  });
}
...
export default async function LessonPage({ params }: LessonPageProps) {
  ...
  const localeP: Locale = await getServerLocale();
  const { t } = getLocale(localeP);
  ...
  if (!curriculum.schemaReady) {
    await redirectLocale(`/tracks/${slug}`);
  }
        ...
        <section className="py-24 md:py-28 px-8">
          <div className="max-w-[980px] mx-auto space-y-8">
            <div className="flex items-center gap-3 flex-wrap">
              <Link
                href={localePath(`/tracks/${track.slug}`, localeP)}
                    ...
                    {curriculum.totalLessons > 0 ? (
                      <div className="flex items-center gap-2.5 mt-2">
                        ...
                        <span className="text-[10px] text-[#89c7ff] font-semibold">
                          {t("lessonPage.lessonCount").replace("{completed}", String(curriculum.completedLessons)).replace("{total}", String(curriculum.totalLessons)).replace("{progress}", String(curriculum.progressPercent))}
                        </span>
                      </div>
                    ) : null}
                  </div>
