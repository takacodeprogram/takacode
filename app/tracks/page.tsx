        ...
        <section className="py-24 md:py-28 px-8">
          <div className="max-w-[1320px] mx-auto space-y-8">
            <div className="max-w-[940px]">
              ...
              <p className="font-body-readable text-[15px] text-[var(--muted-3)] mt-4">
            ...
            {allTracks.length ? (
              <section className="rounded-2xl border border-[var(--border-3)] bg-[var(--surface-1)] p-5 md:p-6">
                <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
                  <h2 className="font-venite-italic text-[16px] text-[var(--text-primary)]">{t("tracksPage.availableTitle")}</h2>
                  <span className="text-[11px] text-[var(--muted-3)] font-body-readable">{t("tracksPage.activeCount").replace("{n}", String(allTracks.length))}</span>
                </div>
                ...
                <div className="rounded-xl border border-blue-500/20 bg-blue-500/[0.06] px-4 py-3 mb-5 flex items-start gap-2.5">
                  ...
                  <p className="font-body-readable text-[12px] text-blue-100/90 leading-relaxed">
                    <span className="font-semibold text-[var(--text-primary)]">{t("tracksPage.orderHint")}</span>
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {allTracks.map((track, index) => {
                    ...
                    return (
                      <article key={track.id} className="rounded-xl border border-[var(--border-3)] bg-[var(--overlay-2)] p-4 card-hover project-card">
                        ...
                        <h3 className="font-venite-italic text-[14px] text-[var(--text-primary)] leading-tight mb-1.5">{track.title}</h3>
                        <p className="font-body-readable text-[11px] text-[var(--muted-4)] mb-3">{metaStr}</p>

                        <div className="flex flex-wrap gap-1.5 mb-4">
                          {competencies.map((competence) => (
                            <span key={`${track.id}-${competence}`} className="text-[10px] px-2 py-1 rounded-full border border-[var(--border-3)] bg-[var(--overlay-3)] text-[var(--muted-2)]">
                              {competence}
                            </span>
                          ))}
                        </div>
