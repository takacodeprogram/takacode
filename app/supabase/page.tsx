...
export default async function SupabasePage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--bg-app)] text-[var(--text-primary)] p-8">
      ...
      {error ? (
        ...
        <ul className="space-y-2 font-body-readable text-sm text-[var(--muted-2)]">
          {todos?.map((todo) => (
            <li key={todo.id} className="rounded-lg border border-[var(--border-3)] bg-[var(--overlay-3)] px-3 py-2">
              {todo.name}
            </li>
          ))}
